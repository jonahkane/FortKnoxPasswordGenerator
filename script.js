...
// it is possible for a user to exclude all character types.  since we require some characters in order
// to create a password, adding an additional IF function to require the user select at least one type of input
if (confirmSymbols != true && confirmDigits != true && confirmLower != true && confirmUpper != true) {
  alert("You must include at least one type of character for your password")
  return writePassword()
}
...
// if the user chose to incorporate them into their password or not.  it will also confirm at the end 
